...
\subsubsection{YARN}
Hadoop YARN (Yet Another Resource Negotiator) is a relatively recent addition to the Hadoop project. YARN is a scheduler and resource negotiation system, allocating tasks to machines on the cluster based on their resources. Resources are represented as an abstract 'container` which has information about the resources of each machine (memory, CPU, etc). Each machine can hold more than one container, and work is assigned on a per-container basis. YARN is also responsible for handling job submission. 
