...
\node[block, solid, minimum width=2.5cm](MR){MapReduce};
\node[block, solid, right=of MR, minimum width=2.5cm](PIG){Pig};
\node[block, solid, right=of PIG, minimum width=2.5cm](HIVE){Hive};

\node[draw, fit=(MR) (PIG) (HIVE), dashed, rounded corners, minimum width=9cm](container){};

\node[right=of container] {Data Processing Tools};
\node[block, below=of container](YARN){YARN};
\node[right=of YARN]{Cluster Management};
\node[block, below=of YARN](HDFS){HDFS};
\node[right=of HDFS] {Storage};
...
\subsubsection{YARN}
Hadoop YARN (Yet Another Resource Negotiator) is a relatively recent addition to the Hadoop project \cite{YARN}. YARN is a scheduler and resource negotiation system, allocating tasks to machines on the cluster based on their resources. Resources are represented as an abstract 'container` which has information about the resources of each machine (memory, CPU, etc). Each machine can hold more than one container, and work is assigned on a per-container basis. YARN is also responsible for handling job submission. 
...
\node[block, solid, minimum width=2.5cm](JavaAPI){Java API};
\node[block, solid, right=of JavaAPI, minimum width=2.5cm](ScalaAPI){Scala API};
\node[block, solid, right=of ScalaAPI, minimum width=2.5cm](Meteor){Meteor};

\node[draw, fit=(JavaAPI) (ScalaAPI) (Meteor), dashed, rounded corners, minimum width=9cm](container1){};

\node[right=of container1] {Client API's};
\node[block, below=of container1](Optimizer){Optimizer};
\node[block, below=of Optimizer](Runtime){Runtime};

\node[block, solid, minimum width=2.5cm, below=of Runtime, yshift=-1.5mm](Direct) {Direct};
\node[block, solid, minimum width=2.5cm, left=of Direct](YARN) {YARN};
\node[block, solid, minimum width=2.5cm, right=of Direct](EC2) {EC2};
\node[draw, fit=(YARN) (Direct) (EC2), dashed, rounded corners, minimum width=9cm](container2) {};
\node[right=of container2] {Cluster Manager};

\node[block, solid, minimum width=2.5cm, below=of container2, yshift=-1.5mm](HDFS) {HDFS};
\node[block, solid, minimum width=2.5cm, left=of HDFS](Local) {Local};
\node[block, solid, minimum width=2.5cm, right=of HDFS](S3) {S3};
\node[draw, fit=(Local) (HDFS) (S3), dashed, rounded corners, minimum width=9cm](container3) {};
\node[right=of container3] {Storage};

\end{tikzpicture}
\caption{Stratosphere Architecture Diagram}
\end{figure}
